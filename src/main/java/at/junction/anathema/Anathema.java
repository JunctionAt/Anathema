...
public class Anathema extends JavaPlugin implements Listener
{
    ...
    public void onAsyncPlayerPreLoginEvent(AsyncPlayerPreLoginEvent event) {
    	...
    	if(bans!=null && bans.size()!=0) {
    		event.disallow(AsyncPlayerPreLoginEvent.Result.KICK_BANNED, bans.get(0).reason + getConfig().getString("message.banappend"));
    		return;
    	}
    ...
    void kickIfOnline(final String player, final String reason) {
        ...
        runTask(new Runnable() {
            public void run() {
                ...
                if(player != null) {
                    playerObj.kickPlayer(reason + getConfig().getString("message.banappend"));
                }
            }
        });
    }
